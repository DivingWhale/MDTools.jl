...
# 字段
- `data::AbstractVector{UInt8}`: 原始字节数据
...
"""
    BitBuffer(data::AbstractVector{UInt8}) -> BitBuffer
...
"""
    reset!(buffer::BitBuffer, data::Vector{UInt8}, len::Int)

重置位缓冲区以使用新数据（避免重新分配）。
"""
@inline function reset!(buffer::BitBuffer, data::Vector{UInt8}, len::Int)
    buffer.data = data
    buffer.index = 1
    buffer.lastbits = 0
    buffer.lastbyte = UInt32(0)
    return buffer
end
